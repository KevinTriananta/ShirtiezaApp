...
import { Search, RefreshCw, PackageCheck, Truck, XCircle } from 'lucide-react';
...
export default function AdminOrdersPage() {
  ...
  const updateStatus = async (order: Order, status: string) => {
    try {
      ...
      const payment_status = status === 'cancelled' ? order.payment_status : 'paid';
                      ...
                      {(order.items || []).map((item) => (
                        <div key={item.id} className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-neutral-400">Qty {item.quantity} x Rp {item.price.toLocaleString('id-ID')}</p>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-end" onClick={() => setSelectedOrder(null)}>
          <div className="bg-white h-full w-full max-w-lg p-8 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="border-t pt-4 space-y-2 text-sm">
              ...
              <p><b>Tracking:</b> SHZ-{String(selectedOrder.id).padStart(5, '0')} (demo)</p>
            </div>
