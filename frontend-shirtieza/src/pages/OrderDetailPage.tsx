...
export default function OrderDetailPage() {
  ...
  const uploadPaymentProof = async () => {
    ...
    if (paymentProof.size > 5 * 1024 * 1024) {
      setUploadError('File terlalu besar. Maksimal ukuran bukti pembayaran adalah 5MB.');
      return;
    }

    try {
      setIsUploading(true);
      const response = await orderService.uploadPaymentProof(order.id, paymentProof);
      setOrder(response.data);
      setPaymentProof(null);
      setUploadSuccess('Bukti pembayaran berhasil diupload. Admin akan memeriksa pembayaran Anda.');
    } catch (error: any) {
      setUploadError(error?.response?.data?.error || error?.response?.data?.message || 'Gagal upload bukti pembayaran. Coba lagi.');
    } finally {
  ...
  const paymentProofUrl = getPaymentProofUrl(order.payment_proof);
  const canUploadPaymentProof = order.payment_status !== 'paid' && order.status !== 'cancelled';
                ...
                {order.items.map((item) => (
                  <div key={item.id} className="flex justify-between items-center py-4 first:pt-0 last:pb-0">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-[11px] text-neutral-400 mt-0.5">Qty: {item.quantity} {[item.size && `Size ${item.size}`, item.color && `Color ${item.color}`].filter(Boolean).join(' / ')}</p>
                      </div>
                    </div>
              ...
              <div className="mt-5 pt-5 border-t border-neutral-100">
                ...
                {paymentProofUrl && (
                  <div className="mt-4 rounded-2xl border border-neutral-100 bg-neutral-50 p-3">
                    <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-neutral-400">Uploaded Proof</p>
                    {order.payment_proof?.toLowerCase().endsWith('.pdf') ? (
                      <a href={paymentProofUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm font-bold text-black underline"><FileText size={16} /> Open PDF proof</a>
                    ) : (
                      <a href={paymentProofUrl} target="_blank" rel="noreferrer" className="block overflow-hidden rounded-xl border border-neutral-200 bg-white">
                        <img src={paymentProofUrl} alt="Payment proof" className="max-h-64 w-full object-contain" />
                      </a>
                    )}
                    <p className="mt-2 text-[11px] text-neutral-500">Status: {order.payment_status === 'waiting_confirmation' ? 'menunggu konfirmasi admin' : order.payment_status}</p>
                  </div>
                )}
                ...
                {canUploadPaymentProof ? (
                  <div className="mt-4 space-y-3">
                    <div className="rounded-2xl border border-dashed border-neutral-200 bg-neutral-50 p-4">
                      <label className="mb-2 block text-[10px] font-black uppercase tracking-widest text-neutral-400">Upload bukti transfer</label>
                      <input type="file" accept="image/png,image/jpeg,image/jpg,image/webp,application/pdf" onChange={(event) => { setPaymentProof(event.target.files?.[0] || null); setUploadError(''); setUploadSuccess(''); }} className="w-full text-xs" />
                      <p className="mt-2 text-[11px] leading-relaxed text-neutral-500">Format: JPG, PNG, WEBP, atau PDF. Maksimal 5MB. Jika pembayaran ditolak, upload ulang bukti yang benar.</p>
                      {paymentProof && <p className="mt-2 text-xs font-bold text-black">Selected: {paymentProof.name}</p>}
                    </div>
                    {uploadError && <div className="flex gap-2 rounded-xl bg-red-50 px-3 py-2 text-xs font-semibold text-red-600"><AlertCircle size={15} /> {uploadError}</div>}
                    {uploadSuccess && <div className="flex gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700"><CheckCircle2 size={15} /> {uploadSuccess}</div>}
                    <button onClick={uploadPaymentProof} disabled={!paymentProof || isUploading} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-black px-4 py-3 text-[11px] font-black uppercase tracking-widest text-white disabled:opacity-50">
                      <Upload size={14} /> {isUploading ? 'Uploading...' : paymentProofUrl ? 'Upload Ulang Bukti' : 'Upload Bukti Pembayaran'}
                    </button>
                  </div>
                ) : (
                  <p className="mt-4 rounded-xl bg-neutral-50 p-3 text-xs text-neutral-500">Upload bukti pembayaran tidak tersedia untuk order yang sudah dibayar atau dibatalkan.</p>
                )}
              </div>
            </div>
          </div>
